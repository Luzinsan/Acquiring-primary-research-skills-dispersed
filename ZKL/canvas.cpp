...
void Canvas::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    {
    case 1:
        *pointsOfLine << event->pos().toPoint(); // Сохраняем координаты точки нажатия
        addEllipse(event->pos().x(), event->pos().y(), 1, 1, QPen(pen->color()));
        break;
   /* case 2:
        QGraphicsItem *item;
        break;
...
void Canvas::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    {
        case 1: // Рисование
            QLine *current_line = new QLine(pointsOfLine->last(), event->scenePos().toPoint());
